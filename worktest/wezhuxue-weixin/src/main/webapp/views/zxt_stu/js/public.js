var pub={
		//服务器前缀
		//api:'http://localhost:8080/zxyp_api/',
		api:'http://192.168.10.33:8080/zxyp_api/web/h5/encapsulation/forward',
		weixinApi:'http://shop.zhuxueup.com/zhuxueup_weixin/authorizeServlet',
		jsonPApiT:'http://123.56.207.57:8080/zxyp_api/web/h5/encapsulation/forward',
		jsonPApi:'https://api.zhuxueup.com/zxyp_api/web/h5/encapsulation/forward',
		wxSignApi:'http://wx.wezhuxue.com/wezhuxue-weixin/jsSdkServlet',
		wxDomain:'http://wx.wezhuxue.com/wezhuxue-weixin/',
		//点击onBtn清空clearVal的值
		/**
		 * onBtn传点击按钮
		 * clearVal传要清空的元素
		...
		//手机号正则
		 vlidateNumber:function(num) {
		    var reg = /^0?1[3|4|5|8][0-9]\d{8}$/;
		    return reg.test(num);
		},
		/**复合遮弹层 composite shelter
		   * 调用sh函数
		   * contents次参数是报错内容
		   * 必须在页面最底部加上<div id="oDiv" style="position: fixed; left: 0px; right: 0px; top: 0px; bottom: 0px; display: none"></div>
		   * */
		cs:function(contents){
			...
			for(var index in oDivStyles)
			{
				oDiv.style[index]=oDivStyles[index];
			}*/
			//document.body.appendChild(oDiv);//把遮层父级加载到body
			//创建遮层背景层
			//var oDiv = document.getElementById('zlDiv');
			...
			//设置遮层背景层样式
			for(var index in oDivBGStyles)
			{
				oDivBG.style[index]=oDivBGStyles[index];
			}
			oDiv.innerHTML = "";
			oDiv.appendChild(oDivBG);//把遮层背景层加入遮层父级
			//创建遮层内容
			
		    if(typeof oDivContent == "undefined"){
				...
				//设置遮层内容层样式
				for(var index in oDivConStyles)
				{
					oDivContent.style[index]=oDivConStyles[index];
				}
				
				//oDivContent.innerHTML='<div style="text-align:center; word-break:break-all;  width:97%; font-size:16px; border-bottom:1px solid #ddd; padding:3%">'+contents+'</div><div style="text-align:center;width:100%; font-size:18px; color:#00f; border-bottom:1px solid #ddd;" ><span id="btn" style="display:block; padding:2%;">确定</span></div>';
				oDivContent.innerHTML='<div style="text-align:center; word-break:break-all; font-size:16px; border-bottom:1px solid #ddd; padding:6% 4%;">'+contents+'</div><div style="text-align:center;width:100%; font-size:18px; color:#00f; border-bottom:1px solid #ddd;" ><span id="btn" style="display:block; padding:3%;color:#ff5544;font-size:15px;">确定</span></div>';
				oDiv.appendChild(oDivContent);//把遮层内容层加入遮层父级
		    }
			
			//点击隐藏
			//console.log(document.getElementById('btn'));
			document.getElementById('btn').onclick=hidden;
			oDivBG.onclick=hidden;
			function hidden(){
		 ...
		 *obj运动的元素
		 * json运动是属性	 
		 * options 运动时间  运动方式  运动后回调json
		 * move(aDiv,{marginRight:500,marginTop:10,opacity:.3},{ duration:1000,easing:Tween.Back.easeInOut})
		 **/
		...
		months 月数
		**/
		credit:function(amount,monthlyRate,months){
		if(months != parseInt(months)){
			throw "hehe";
		}
		var yearlyRate = monthlyRate*12,
			repaymentByMonth = amount*monthlyRate*Math.pow(1+monthlyRate,months)/(Math.pow(1+monthlyRate,months)-1),
			allrepayment = repaymentByMonth*months,
			allRate = allrepayment - amount,
			rateByMonth = allRate/months;
		return {
			yearlyRate : yearlyRate,//年利率
			repaymentByMonth : repaymentByMonth.toFixed(2),//月还款
			allrepayment : allrepayment.toFixed(2),//所有总还款
			allRate : allRate.toFixed(2),//所有还利率
			rateByMonth : rateByMonth.toFixed(2),//月均还利率
			realRateByMonth : function(month){//某月真实利率 传入该月 应该是用不到
				...
				return (amount*monthlyRate - repaymentByMonth)*Math.pow(1+monthlyRate,month-1)+repaymentByMonth;
				}
			}
	},
	/**
	 ...
	 * 解密
	 * str传加密后的字符串
	 * 字符串格式'23698,21150,23698'
	 * 原理把加密后的str转成数组然后转成需要的字符串
	 * **/
	fcc:function(str)
	{
		if(str!=null)
	 ...
	 *  ***从初始数到结束数 的数字滚动 ***
	 * domObj  要操作的元素
	 * json:{starNum,callbackTime,endNum，fn}
	 * starNum 初始值 (选填)
	 * callackTime 回调时间(选填)
	 * endNum 最终值(必填)
	 * fn 选填
	...
	isPhoneNum:function(phone){
		var reg=/^((1[3,5,8][0-9])|(14[5,7])|(17[0,6-8]))\d{8}$/;
	 ...
	 * 返回1验证通过
	 **/
	studentID:function(num){
		var reg = /^[19][0-9]20\d{2}[0-3][0-5]$/;
		if(reg.test(num)===false)
		{
			console.log("请输入您的正确学号！");
			return 0;
		}else{
			return 1;
		}
	 ...
	 * 活动弹窗
	 * **/
	errorAlert:function (zlErr,aBtn){
			var zlABtn=$('#zlABtn');
			aBtn=aBtn || '确定';
			$('#CoverLayer').css('display','block');
			$('#zlErr').html(zlErr);
			zlABtn.html(aBtn);
			zlABtn.one('click', function (){
				if(zlABtn.html()==='去下载'){
					alert(pub.isAI());	
				}else{
	 ...
	 *months 月数
	**/
	credit2:function(amount,monthlyRate,months){
		if(months != parseInt(months)){
			throw "hehe";
		}
		var Totalinterest = amount*monthlyRate*months+amount,// 总利息+借款金额
			rateByMonth = amount*monthlyRate,//月利息
			Mpayments=Totalinterest/months;//月还款
		return {
			Totalinterest : Totalinterest.toFixed(2),//总利息
			rateByMonth : rateByMonth.toFixed(2),//月利息
			Mpayments : Mpayments.toFixed(2),//月还款
			realRateByMonth : function(month){//某月真实利率 传入该月 应该是用不到
				if(month != parseInt(month)){
					throw "hehe";
				}
	...
	**/
	...
	openApp:function(e){
		e.preventDefault();
    	var u = navigator.userAgent;
	    if(pub.isWeixinBrowser() || pub.isQQBrowser()){
	        //友好的提示页面
	        window.location = pub.wxDomain+'views/zxt_stu/openApp.html';
	    }else{
	        if(u.indexOf('Android') > -1 || u.indexOf('Linux') > -1){
	            //android
	            //此操作会调起app并阻止接下来的js执行
	            $('body').append("<iframe src='wezx://' style='display:none' target='' ></iframe>");
	            //没有安装应用会执行下面的语句
	            setTimeout(function(){window.location = pub.wxDomain+'views/zxt_stu/downLoadAIApp.html'},2000);
	        }else{
	            //ios
	            window.location = 'wezx://';
	            setTimeout(function(){window.location = 'itms-apps://itunes.apple.com/app/id1188943755'},2000);
	        }
	    }
	}
};